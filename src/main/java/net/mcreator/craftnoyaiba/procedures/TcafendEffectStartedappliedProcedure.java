				...
				synchronized (connections) {
					...
					while (iterator.hasNext()) {
						...
						if (!connection.isConnecting() && connection.isConnected())
							CraftnoyaibaMod.PACKET_HANDLER.sendTo(new SetupAnimationsProcedure.CraftnoyaibaModAnimationMessage(Component.literal("thunderclapandflashend"), entity.getId(), false), connection, NetworkDirection.PLAY_TO_CLIENT);
					}
				}
			}
		}
				...
				if (entity instanceof Player && world instanceof ServerLevel srvLvl_) {
					...
					synchronized (connections) {
						...
						while (iterator.hasNext()) {
							Connection connection = iterator.next();
							if (!connection.isConnecting() && connection.isConnected())
								CraftnoyaibaMod.PACKET_HANDLER.sendTo(new SetupAnimationsProcedure.CraftnoyaibaModAnimationMessage(Component.literal("defaultend"), entity.getId(), false), connection, NetworkDirection.PLAY_TO_CLIENT);
						}
					}
				}
			}
