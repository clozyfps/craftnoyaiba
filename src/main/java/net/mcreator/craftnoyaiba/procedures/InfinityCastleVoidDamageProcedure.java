...
public class InfinityCastleVoidDamageProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, DamageSource damagesource, Entity entity) {
		...
		if (damagesource.is(DamageTypes.FELL_OUT_OF_WORLD)) {
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = CraftnoyaibaModEntities.INFINITY_CASTLE_DOOR_UPSIDE_DOWN.get().spawn(_level, BlockPos.containing(x, y - 2, z), MobSpawnType.MOB_SUMMONED);
				if (entityToSpawn != null) {
					entityToSpawn.setYRot(world.getRandom().nextFloat() * 360F);
				}
			}
			if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
				_entity.addEffect(new MobEffectInstance(MobEffects.REGENERATION, 20, 250, false, false));
		}
	}
}
