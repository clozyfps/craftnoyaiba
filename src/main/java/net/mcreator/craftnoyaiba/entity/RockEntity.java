...
import net.mcreator.craftnoyaiba.procedures.RockWhileProjectileFlyingTickProcedure;
...
public class RockEntity extends AbstractArrow implements ItemSupplier {
	...
	public RockEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(CraftnoyaibaModEntities.ROCK.get(), world);
	}
	...
	public void tick() {
		super.tick();
		RockWhileProjectileFlyingTickProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
		if (this.inGround)
			this.discard();
	}
	...
	public static RockEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		RockEntity entityarrow = new RockEntity(CraftnoyaibaModEntities.ROCK.get(), entity, world);
	...
	public static RockEntity shoot(LivingEntity entity, LivingEntity target) {
		RockEntity entityarrow = new RockEntity(CraftnoyaibaModEntities.ROCK.get(), entity, entity.level());
